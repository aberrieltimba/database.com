...
var Tooltip = {
	...
	/**
	 * @var object suspendHide Tooltip
	 */
	suspendHide: false,
	...
	{	
		...
		try {
			if (activator.Tooltip.isVisible) {
				if(!Tooltip.suspendHide) Tooltip._hide(activator, event);
			} else {
    ...
    * @private
    ...
    _follow: function (activator, event)
    {
      ...
      activator.Tooltip.style.position = "absolute";
		...
		//alert(document.styleSheets[1].rules[".sfdctip"].selectorText); //.style.marginTop);
		//alert(activator.Tooltip.offsetHeight + " -- " + activator.Tooltip.offsetWidth);

		var availH = top - document.body.scrollTop ;
		tt_h = 260;
		if((availH - tt_h) >=0){
			activator.Tooltip.className = "sfdctip";
		}else{
			activator.Tooltip.className = "sfdctip_d";
		}
